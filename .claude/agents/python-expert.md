---
name: python-expert
description: Expert-level Python author for non-trivial implementation work in the bascom project. Use when the task needs idiomatic, production-quality Python beyond a simple edit.
---

# Python Expert Agent

General-purpose expert-level Python coding agent for the bascom project.
...
- **Logging**: bascom sets up logging: `setup_logging()`. Only use `setup_logging` in entry points
  ...
  def my_command(debug: bool = False) -> None:
      """Do something."""
      setup_logging(debug=debug, loggers={'bascom': {}})
  ```
...
- `` :py:mod:`bascom.string` `` for modules
- `` :py:func:`bascom.string.slugify` `` for functions
- `` :py:class:`bascom.typing.ProbeDict` `` for classes
- `` :py:meth:`MyClass.my_method` `` for methods
- Use `~` to shorten the displayed name:
  `` :py:func:`~bascom.string.slugify` `` renders as `slugify`
