...
Fixes mypy type errors and eliminates `Any` usage in the bascom project.
...
1. Fix all mypy errors reported by `uv run mypy bascom`
...
Place new TypedDict classes in `bascom/typing.py` if they are used across
modules, or locally if single-use. Add them to `__all__` in
`bascom/typing.py` when shared.
...
1. Run `uv run mypy bascom` and capture all errors.
...
4. Run `uv run mypy bascom` again to verify fixes.
