...
If any changed files are under `bascom/` or `tests/`, run the following agents **in order**:
...
- **changelog** - update `CHANGELOG.md` with entries for the changes. After it completes, check if
  ...
  the relevant commit. **Only run when changes affect users**: files under `bascom/`, `tests/`,
...
For example, if a commit contains `bascom/commands/main.py`,
`tests/test_main_command.py`, and `CHANGELOG.md`, the component is determined by
`bascom/commands/main.py` and `tests/test_main_command.py` only.
...
- Changes span unrelated components (e.g. `bascom/media.py` and `.claude/agents/release.md`).
...
For Python files, strip the `bascom/` prefix and replace `/` with `.` (like module imports).

- Python file `bascom/media.py` → `media:`.
- Multiple files under `bascom/commands/` → `commands:`.
- Single command file `bascom/commands/admin.py` → `commands.admin:`.
